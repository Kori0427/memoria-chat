...
function renderMemoryForPrompt(store) {
  ...
  if (safe.identity.length > 0) {
    sections.push(
      `## 核心身份\n${safe.identity.map((item) => `- ${item.text} [${item.date}]`).join("\n")}`
    );
  }

  if (safe.preferences.length > 0) {
    sections.push(
      `## 偏好习惯\n${safe.preferences.map((item) => `- ${item.text} [${item.date}]`).join("\n")}`
    );
  }

  if (safe.events.length > 0) {
    sections.push(
      `## 近期动态\n${safe.events.map((item) => `- ${item.text} [${item.date}]`).join("\n")}`
    );
  }
...
function selectMemoryForPrompt(store, budget = MEMORY_TOKEN_BUDGET) {
  ...
  const sortByDateDesc = (a, b) => (b.date > a.date ? 1 : b.date < a.date ? -1 : 0);

  const identity = safe.identity; // 保持原始顺序
  const preferences = [...safe.preferences].sort(sortByDateDesc);
  const events = [...safe.events].sort(sortByDateDesc);
  ...
  // Phase 2: preferences 按日期新→旧，逐条检查预算
  ...
  // 渲染为 Markdown（格式同 renderMemoryForPrompt）
  const sections = [];
  if (selected.identity.length > 0) {
    sections.push(
      `## 核心身份\n${selected.identity.map((i) => `- ${i.text} [${i.date}]`).join("\n")}`
    );
  }
  if (selected.preferences.length > 0) {
    sections.push(
      `## 偏好习惯\n${selected.preferences.map((i) => `- ${i.text} [${i.date}]`).join("\n")}`
    );
  }
  if (selected.events.length > 0) {
    sections.push(
      `## 近期动态\n${selected.events.map((i) => `- ${i.text} [${i.date}]`).join("\n")}`
    );
  }
...
function renderMemoryWithIds(store) {
  ...
  const safe = validated.value;
  const sections = [];
  const categoryNames = { identity: "核心身份", preferences: "偏好习惯", events: "近期动态" };
...
async function buildSystemPrompt(config) {
  ...
  const memory = selectMemoryForPrompt(store);
