...
const OPENROUTER_APP_NAME = (process.env.OPENROUTER_APP_NAME || "memoria-chat").trim();
...
const hasAnyProvider = !!(openaiClient || arkClient || openrouterClient);
if (!hasAnyProvider) {
  console.warn("⚠️  未配置任何 API Key（OPENAI_API_KEY / ARK_API_KEY / OPENROUTER_API_KEY），聊天功能不可用。");
}
...
function getClientForModel(model) {
  ...
  const isArk = /^(glm|kimi|doubao|moonshot)/i.test(model);
  if (isArk) {
    if (!arkClient) throw new Error("未配置 ARK_API_KEY，无法使用火山引擎模型");
    return arkClient;
  }
  // 自定义 OPENAI_BASE_URL（如 Ollama）时，未识别的模型走 openaiClient
  if (hasCustomOpenAIBase && openaiClient) return openaiClient;
  if (!arkClient) throw new Error("未配置 ARK_API_KEY，无法使用火山引擎模型");
  return arkClient;
}
