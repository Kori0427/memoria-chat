...
const zh = {
  ...
  err_no_api_keys: "未检测到 API Key 配置。\n\n请编辑 .env 文件，添加至少一个 API Key（OPENAI_API_KEY / ARK_API_KEY / OPENROUTER_API_KEY），然后重启服务。\n\n点击「确定」继续浏览（聊天功能暂不可用）。",
  ...
  voice_stt_groq: "Groq Whisper (免费)",
...
const en = {
  ...
  err_no_api_keys: "No API keys detected.\n\nPlease edit the .env file and add at least one API key (OPENAI_API_KEY / ARK_API_KEY / OPENROUTER_API_KEY), then restart the server.\n\nClick OK to continue browsing (chat features will be unavailable).",
  ...
  voice_stt_api: "OpenAI Whisper API",
  voice_stt_groq: "Groq Whisper (free)",
